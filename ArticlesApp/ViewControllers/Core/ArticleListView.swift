...
struct ArticleListView: View {
    ...
    @State private var viewMode: ViewMode = .list   // ← start in LIST mode
    ...
    // MARK: - Nav Bar (bold Articles + teal icons)
    ...
    // MARK: - Offline (Figma: grey bg, wifi.slash, retry capsule)
    private var offlineView: some View {
        VStack(spacing: 20) {
            ...
            Button {
                ...
                .padding(.vertical, 13)
                .overlay(Capsule().stroke(Color(UIColor.label).opacity(0.35), lineWidth: 1.5))
            }
            Spacer()
        }
